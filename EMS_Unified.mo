 ...
 */
  ...
  // ===== 외부 입력 =====
  // P_dem 입력 제거 - 내부에서 P_demand로 계산됨
 ...
 // ===== 컨트롤 버스 (부하/소스) =====
  ...
  .Electrification.Control.Signals.BusSelector busLoad(
    category=Electrification.Utilities.Types.ControllerType.Load, id=load_id)
    annotation (Placement(transformation(origin={0.0,0.0}, extent={{22.87,58.06},{10.87,70.06}},rotation = 0.0)));
...
protected
  ...
  // 조건부 신호 상태를 저장하는 변수들 (참조용)
  Boolean pMaxOutActive[N], pMaxInActive[N];
  Boolean iMaxOutActive[N], iMaxInActive[N];
 ...
 // === 내부 모드별 P_dem 생성 ===
  ...
  // 간단한 슬루 레이트 리미터 (RateLimiter 블록 사용)
  .Modelica.Blocks.Nonlinear.SlewRateLimiter powerSlew(
    Rising=ramp_up,   // 상승 슬루 레이트 [W/s]
    Falling=ramp_dn,  // 하강 슬루 레이트 [W/s]
    y_start=0)
    annotation (Placement(transformation(extent={{-50,-40},{-30,-20}})));
    
  SI.Power P_mag_shaped;  // 필터링된 전력
  SI.Power P_clamp;
  ...
  // srcLimits connector → Real 변수 연결용 PassThrough 블록들
  .Modelica.Blocks.Routing.RealPassThrough pMaxOutPass[N]
    annotation (Placement(transformation(extent={{-70,0},{-50,20}})));
  .Modelica.Blocks.Routing.RealPassThrough pMaxInPass[N]
    annotation (Placement(transformation(extent={{-70,-20},{-50,0}})));
  .Modelica.Blocks.Routing.RealPassThrough iMaxOutPass[N]
    annotation (Placement(transformation(extent={{-70,-40},{-50,-20}})));
  .Modelica.Blocks.Routing.RealPassThrough iMaxInPass[N]
    annotation (Placement(transformation(extent={{-70,-60},{-50,-40}})));

  // 소스별 슬루 레이트 제한 블록들
  .Modelica.Blocks.Nonlinear.SlewRateLimiter sourceSlew[N](
    Rising=dP_rise,   // 파라미터 배열 사용
    Falling=dP_fall,  // 파라미터 배열 사용
    each y_start=0)
    annotation (Placement(transformation(extent={{60,40},{80,60}})));

equation
  // === srcLimits에서 connector 연결로 한계값 상속 ===
  for i in 1:N loop
    // 신호 상태 저장 (참조용)
    pMaxOutActive[i] = srcLimits[i].pMaxOutSignal;
    pMaxInActive[i] = srcLimits[i].pMaxInSignal;
    iMaxOutActive[i] = srcLimits[i].iMaxOutSignal;
    iMaxInActive[i] = srcLimits[i].iMaxInSignal;
    
    // connector → PassThrough → Real 변수 연결
  ...
  connect(busLoad.systemBus, controlBus)
    annotation (Line(points={{10.87,64.06},{0,64.06},{0,100}}, color={240,170,40}, pattern=LinePattern.Dot, thickness=0.5));
  connect(setLoad.componentBus, busLoad.componentBus)
    annotation (Line(points={{97.11,10.76},{105.89,10.76},{105.89,64.06},{22.87,64.06}}, color={240,170,40}, pattern=LinePattern.Dot, thickness=0.5));
  connect(vipLoad.componentBus, busLoad.componentBus)
    annotation (Line(points={{78.29,-88.78},{105.11,-88.78},{105.11,64.06},{22.87,64.06}}, color={240,170,40}, pattern=LinePattern.Dot, thickness=0.5));
...
//  V_bus = max(Veps, if v_bus_in > 0 then v_bus_in else vipLoad.v);
  ...
  // === 부하 허용합 + SetComponentLimits로 충전기에 전달
  p_max_out_load = sum(cap_out[i] for i in 1:N);
  p_max_in_load  = sum(cap_in[i]  for i in 1:N);

  setLoad.p_max_out = p_max_out_load;
  setLoad.p_max_in = p_max_in_load;
  
  // 전압 한계: 소스들의 범위 기반
  setLoad.v_max = max(vipSrc[i].v for i in 1:N) + 50.0;  // 최대 + 여유
  setLoad.v_min = min(vipSrc[i].v for i in 1:N) - 50.0;  // 최소 - 여유
  
  // 전류 한계: 전력/전압 기반
  setLoad.i_max_out = p_max_out_load / max(Veps, V_bus);
  setLoad.i_max_in = p_max_in_load / max(Veps, V_bus);
  ...
  // 충전 전력 계산
  P_mag_raw = k_total * P_nom;  // 스케일링된 기본 전력
  P_from_i = if iMaxIn_opt < 1e8 then min(P_mag_raw, V_bus * iMaxIn_opt) else P_mag_raw;  // 전류 제한 적용
  P_mag_capped = min(P_from_i, P_charger_cap);  // 충전기 용량 제한 적용

  // SlewRateLimiter 블록으로 슬루 레이트 제한
  powerSlew.u = P_mag_capped;     // 목표값 입력
  P_mag_shaped = powerSlew.y;     // 슬루 제한된 출력
