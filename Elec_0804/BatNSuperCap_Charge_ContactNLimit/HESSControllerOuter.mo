...
model HESSControllerOuter
  ...
  .Modelica.Blocks.Continuous.Filter filter(f_cut=0.05)
    annotation (Placement(transformation(extent={{46.0,-10.0},{66.0,10.0}},rotation = 0.0,origin = {0.0,0.0})));
...
equation
  ...
  connect(add.y, limiter.u)
    annotation (Line(points={{7,0},{7,0.32},{14.32,0.32}},  color={0,0,127}));
  ...
  connect(filter.y, pwr_a_ref.u_r)
    annotation (Line(points={{67,0},{74,0}}, color={0,0,127}));
  connect(filter.u, limiter.y)
    annotation (Line(points={{44,0},{37.32,0},{37.32,0.32}}, color={0,0,127}));
