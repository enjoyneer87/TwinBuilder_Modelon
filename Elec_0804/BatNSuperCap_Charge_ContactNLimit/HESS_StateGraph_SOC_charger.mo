...
equation
    ...
    connect(vInitA2.plug_a,batty.plug_a) annotation(Line(points = {{-88.87,-147.81},{-57.76,-147.81},{-57.76,-100.05},{-84.14,-100.05}},color = {255,128,0}));
    connect(i_cell.systemBus,batty.controlBus) annotation(Line(points = {{-55.05,48.82},{-55.05,-1.08},{-87.34,-1.08},{-87.34,-84.05}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(batteryChargerFixedLimits_CC_.plug_a,dCSensor.plug_b) annotation(Line(points = {{64.11,-8.5},{64.11,-8.01},{18.22,-8.01}},color = {255,128,0}));
    connect(batteryChargerFixedLimits_CC_.plug_a,batty.plug_a) annotation(Line(points = {{64.11,-8.5},{27.7,-8.5},{27.7,-100.05},{-84.14,-100.05}},color = {255,128,0}));
    connect(dCSensor.y[3],add.u2) annotation(Line(points = {{8.22,-17.01},{8.22,-126},{-320.26,-126},{-320.26,-82},{-314,-82}},color = {0,0,127}));
    ...
    connect(close_contactors.systemBus,batty.controlBus) annotation(Line(points = {{-91.59,157.62},{-91.59,-84.05},{-87.34,-84.05}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(batty.controlBus,soc_Batt.systemBus) annotation(Line(points = {{-87.34,-84.05},{-87.34,-78.05},{-144.88,-78.05},{-144.88,-100.21},{-162.78,-100.21}},color = {240,170,40},pattern = LinePattern.Dot));
