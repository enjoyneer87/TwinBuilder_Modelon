...
model BatFromCar_SuperCapContactNLimit "A battery pack for an electric car"
  extends .Electrification.Batteries.Templates.BatteryPackLumped(
    ns=100,
    np=60,
    redeclare .Electrification.Batteries.Core.Examples.NCA.Characteristic core(redeclare replaceable .Electrification.Batteries.Core.Records.Summary summary),redeclare replaceable .Electrification.Batteries.Electrical.Pack.Contactors electrical,
    redeclare replaceable .Electrification.Batteries.Thermal.Examples.Lumped thermal(C_cell=50, C_pack=0),redeclare replaceable .BatNSuperCap_Charge_ContactNLimit.BatLimitsAndContactors controller(contactors(contactors_closed = false)));
annotation (
...
end BatFromCar_SuperCapContactNLimit;
