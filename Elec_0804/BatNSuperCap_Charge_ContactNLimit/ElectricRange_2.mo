...
model ElectricRange_2 "Range of battery electric vehicle for a specific drive cycle"
  ...
  .Electrification.Batteries.Examples.Applications.ElectricCar battery(
    ...
    limitActionSoC=.Modelon.Types.FaultAction.Terminate)
      annotation (Placement(transformation(extent={{-50.56,-15.28},{-82.56,16.72}},rotation = 0.0,origin = {0.0,0.0})));
  ...
  .Electrification.Machines.Examples.Applications.ElectricCar machine(
    ...
    redeclare replaceable .Electrification.Machines.Control.LimitedTorque controller(
      ...
      torqueControl(external_torque=true),iMaxIn = battery.controller.iMaxIn,iMaxOut = battery.controller.iMaxIn,pMaxIn = battery.controller.pMaxIn,pMaxOut = battery.controller.pMaxIn)) annotation (
        Placement(transformation(extent={{-12.47,-15.53},{19.53,16.47}},rotation = 0.0,origin = {0.0,0.0})));
  ...
  .Electrification.Electrical.DCInit vInitA(init_steady=true, stateSelect=StateSelect.avoid) annotation (
    Placement(transformation(extent={{-70.28,-60.0},{-50.28,-40.0}},rotation = 0.0,origin = {0.0,0.0})));
...
equation
  ...
  connect(vInitA.plug_a, battery.plug_a) annotation (Line(
      points={{-50.28,-50},{-30,-50},{-30,0.72},{-50.56,0.72}},
      color={255,128,0},
      thickness=0.5));
  connect(machine.plug_a, battery.plug_a) annotation (Line(
      points={{-12.47,0.47},{-50.56,0.47},{-50.56,0.72}},
      color={255,128,0},
      thickness=0.5));
  connect(tau_ref.systemBus, machine.controlBus) annotation (Line(
      points={{0,50},{-20,50},{-20,16.47},{-9.27,16.47}},
      color={240,170,40},
  ...
  connect(battery.controlBus, machine.controlBus) annotation (Line(
      points={{-53.76,16.72},{-53.76,16.47},{-9.27,16.47}},
      color={240,170,40},
      pattern=LinePattern.Dot,
      thickness=0.5));
  connect(torqueArbitration.tau_ref_rear[1], tau_ref.u_r) annotation (Line(points={{29,50},{10,50}}, color={0,0,127}));
  connect(machine.flange, chassis.flangeR) annotation (Line(points={{19.53,0.47},{19.53,0},{60,0}}, color={0,0,0}));
