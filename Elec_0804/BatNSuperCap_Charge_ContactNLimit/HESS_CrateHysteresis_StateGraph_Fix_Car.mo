...
model HESS_CrateHysteresis_StateGraph_Fix_Car
  ...
  .BatNSuperCap_Charge_ContactNLimit.BatFromCar_ContactNLimit battery(
    ...
    SOC_start = 0.2,redeclare replaceable .BatNSuperCap_Charge_ContactNLimit.BatLimitsAndContactors controller(redeclare replaceable .Electrification.Batteries.Control.LimitsFixed limits(iMaxOut = 100,iMaxIn = 100,theoretical_limit = true,enable_thermal = false),contactors(preChargeOverlap = 0.1)),
  ...
  .Electrification.Electrical.DCSensor dCSensor
    ...
    .Electrification.Utilities.Blocks.Driver driver(k = 1,Ti = 2,torque_output = false,redeclare replaceable .Electrification.Utilities.DriveCycles.AccelerateDecelerate driveCycle,repeat = true) annotation(Placement(transformation(extent = {{-413.35,-37.46},{-389.97,-17.46}},rotation = 0.0,origin = {0.0,0.0})));
    ...
    .Electrification.Control.Limits.ListenToLimits listenToLimits(typeListen = Electrification.Utilities.Types.ControllerType.Battery,id_listen = SuperCap.id,vMaxSignal = true,vMinSignal = true,iMaxOutSignal = true,iMaxInSignal = true,pMaxOutSignal = true,pMaxInSignal = true) annotation(Placement(transformation(extent = {{-208.0,-192.0},{-188.0,-172.0}},origin = {0.0,0.0},rotation = 0.0)));
    .Electrification.Control.Interfaces.SystemBus controlBus annotation(Placement(transformation(extent = {{-246,-164},{-206,-124}},origin = {0,0},rotation = 0)));

equation
  connect(dCSensor.plug_b,battery.plug_a)
    annotation(Line(points = {{367.48,-99.4},{380,-99.4},{380,-98}},color = {255,128,0}));
    ...
    connect(dCSensor.y[2],abs2.u) annotation(Line(points = {{357.48,-108.4},{357.48,-133.01},{-163.29,-133.01},{-163.29,-92.76},{-157.29,-92.76}},color = {0,0,127}));
    ...
    connect(batteryDCDC.controlBus,battery.controlBus) annotation(Line(points = {{320,-88},{320,-70.96},{382,-70.96},{382,-88}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(mode_ref.systemBus,batteryDCDC.controlBus) annotation(Line(points = {{29.01,-174.12},{34.87,-174.12},{34.87,-266},{320,-266},{320,-88}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(dCSensor2.plug_a,CapDCDC.plug_b) annotation(Line(points = {{347.46,-53.4},{347.46,-54},{322,-54}},color = {255,128,0}));
    connect(SuperCap.controlBus,CapDCDC.controlBus) annotation(Line(points = {{379.46,-43.4},{379.46,-37.81},{304,-37.81},{304,-44}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(dCSensor.y[1],less_4Voltage.u1) annotation(Line(points = {{357.48,-108.4},{357.48,-202},{-47.29,-202},{-47.29,-186.83},{-40.77,-186.83}},color = {0,0,127}));
    ...
    connect(CV.active,booleanToInteger.u) annotation(Line(points = {{83,47},{83,-52.18},{-8.3,-52.18},{-8.3,-174.12},{-2.67,-174.12}},color = {255,0,255}));
    ...
    connect(batteryDCDC.plug_a,dCSensor.plug_a) annotation(Line(points = {{322,-98},{347.48,-98},{347.48,-99.4}},color = {255,128,0}));
    connect(v_a_ref.systemBus,batteryDCDC.controlBus) annotation(Line(points = {{279.96,-112.94},{285.96,-112.94},{285.96,-82.76},{320,-82.76},{320,-88}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(vInitB.plug_a,batteryDCDC.plug_a) annotation(Line(points = {{331.77,-153},{337.77,-153},{337.77,-98},{322,-98}},color = {255,128,0}));
    ...
    connect(CapDCDC.plug_a,machine.plug_a) annotation(Line(points = {{302,-54},{10.44,-54},{10.44,-78},{-281.3,-78}},color = {255,128,0}));
    connect(batteryDCDC.plug_b,machine.plug_a) annotation(Line(points = {{302,-98},{10.38,-98},{10.38,-78},{-281.3,-78}},color = {255,128,0}));
    connect(listenToLimits.signalsBus,controlBus) annotation(Line(points = {{-198,-192},{-198,-198},{-226,-198},{-226,-144}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(machine.controlBus,controlBus) annotation(Line(points = {{-285.04,-62},{-285.04,-56},{-226,-56},{-226,-144}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(SuperCap.controlBus,listenToLimits.systemBus) annotation(Line(points = {{379.46,-43.4},{379.46,-37.4},{-198,-37.4},{-198,-172}},color = {240,170,40},pattern = LinePattern.Dot));
...
end HESS_CrateHysteresis_StateGraph_Fix_Car;
