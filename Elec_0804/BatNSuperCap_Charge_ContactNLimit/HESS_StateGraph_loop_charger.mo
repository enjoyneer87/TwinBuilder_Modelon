...
model HESS_StateGraph_loop_charger "Range of SCap electric vehicle for a specific drive cycle"
  ...
  .Electrification.Electrical.DCInit vInitA(init_steady=false, stateSelect=StateSelect.default,init_start = true,v_start = SCap.summary.ocv,common_mode = false) annotation (
    ...
    .Modelica.Blocks.Logical.Hysteresis soc_high(uHigh = 0.92,uLow = 0.9) annotation(Placement(transformation(extent = {{-10.0,-10.0},{10.0,10.0}},origin = {48.84,563.76},rotation = 90.0)));
    ...
    .Modelica.Blocks.Logical.Switch switch annotation(Placement(transformation(extent = {{-38.49,60.47},{-18.49,80.47}},origin = {0,0},rotation = 0)));
...
equation
    ...
    connect(vInitA.plug_a,SCap.plug_a) annotation(Line(points = {{-151.82,-53.73},{-145.28,-53.73},{-145.28,-7.9},{-157.29,-7.9}},color = {255,128,0}));
    connect(averageStepUpDownDCDC.controlBus,SCap.controlBus) annotation(Line(points = {{-114,2.87},{-114,20.35},{-159.29,20.35},{-159.29,2.1}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(close_contactorsSCcap.systemBus,SCap.controlBus) annotation(Line(points = {{-30.5,250.35},{-30.5,39.3},{-159.29,39.3},{-159.29,2.1}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(averageStepUpDownDCDC.plug_b,SCap.plug_a) annotation(Line(points = {{-132,-7.13},{-132,-7.9},{-157.29,-7.9}},color = {255,128,0}));
    connect(averageStepUpDownDCDC.plug_a,dCSensor.plug_a) annotation(Line(points = {{-112,-7.13},{-1.78,-7.13},{-1.78,-8.01}},color = {255,128,0}));
    ...
    connect(mode_ref.systemBus,averageStepUpDownDCDC.controlBus) annotation(Line(points = {{-120.2,72.83},{-114,72.83},{-114,2.87}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(soc_Batt.y_r,soc_high.u) annotation(Line(points = {{6.02,509.69},{48.84,509.69},{48.84,551.76}},color = {0,0,127}));
    connect(not_soc_high.u,soc_high.y) annotation(Line(points = {{48.56,592.71},{48.56,583.74},{48.84,583.74},{48.84,574.76}},color = {255,0,255}));
    ...
    connect(SCap.controlBus,soc_SC.systemBus) annotation(Line(points = {{-159.29,2.1},{-208.26,2.1},{-208.26,246.01},{-214.81,246.01}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(gain.y,switch.u3) annotation(Line(points = {{-24.6,28.1},{-18.93,28.1},{-18.93,56.47},{-40.49,56.47},{-40.49,62.47}},color = {0,0,127}));
    connect(gain2.y,switch.u1) annotation(Line(points = {{-104.46,118.83},{-40.49,118.83},{-40.49,78.47}},color = {0,0,127}));
    ...
    connect(v_cell_SC.systemBus,SCap.controlBus) annotation(Line(points = {{-148.83,124.99},{-159.29,124.99},{-159.29,2.1}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(Use_Supercap.active,switch.u2) annotation(Line(points = {{-121.05,449.24},{-121.05,70.47},{-40.49,70.47}},color = {255,0,255}));
...
end HESS_StateGraph_loop_charger;
