block ChargerCCCVRef "충전모드 CCCV Power Ref (헤드룸 반영)"
...
// 한계/측정(헤드룸 계산용)
Modelica.Blocks.Interfaces.RealInput cap_bt_in "배터리 충전 cap[W]" annotation(Placement(transformation(extent={{-140,-40},{-100,0}})));
...
Modelica.Blocks.Continuous.PI cvPI(k=kp_v, Ti=Ti_v,
...
// 헤드룸(cap - used)
p_bt_in_eff = max(0, cap_bt_in - max(0, -p_bt_meas));
i_bt_in_eff = max(0, i_max_in_bt - max(0, -i_bt_meas));


// CC: I_CC를 헤드룸으로 제한
i_cmd_cc = min(I_CC, i_bt_in_eff, p_bt_in_eff / max(1e-3, V_bus));


// CV: 전압 오차 PI→전류, 헤드룸 제한
cvPI.u = V_CV - v_pack; // e_v
i_cmd_cv_unclamped = max(0, cvPI.y);
i_cmd_cv = min(I_CC, i_bt_in_eff, p_bt_in_eff / max(1e-3, V_bus), i_cmd_cv_unclamped);
