...
model DischargeCharge_Verification_SCap
  extends  .Electrification.Batteries.Experiments.Verification.Templates.DischargeCharge(
    redeclare replaceable .HESS.KETI_SCapPack batteryPack,
    load(fixed_temperature = false,controller(iDischarge = 10 * batteryPack.i_nom_1C,repeat = true)));
annotation (
  experiment(StopTime=40000, Interval=40, Tolerance=0.000001,StartTime = 0,__Dymola_NumberOfIntervals = 0,__Dymola_fixedstepsize = 0.01),
  Documentation(
...
end DischargeCharge_Verification_SCap;
