...
model Cycle_HESS_EMS_charge_wCommu "Range of SCap electric vehicle for a specific drive cycle"
  import Modelica.StateGraph.*;
  import Modelica.Blocks.Interfaces.*;

      // ---- Parameters (전환 힌트용, 실제 값은 상위에서 결정/생성해 연결) ----
  parameter Real dwell_precharge = 0.5  "s, 프리차지 완료 dwell(예: 릴레이 안정)";
  parameter Real dwell_mode      = 0.3  "s, 모드 전환 최소 체류";
  parameter Boolean start_in_idle = true "초기 Idle 시작 여부";
    
      // ---- External condition ports (상위에서 생성해서 넣는 불리언) ----
  // 프리차지/하드웨어 상태
  BooleanInput plugIn            "외부 DC 충전 커넥터 체결 상태(통신 OK 포함)";
  BooleanInput prechargeOK       "프리차지 완료(버스 ΔV OK)";
  BooleanInput contactorsOK      "메인 컨택터 닫힘 OK";
  BooleanInput faultAny          "임계 Fault(OV/UV/OT/단락/절연 등)";
  BooleanInput clearFault        "Fault 해제 허용";
  ...
  BooleanInput clearFault        "Fault 해제 허용";
    ...
    // 차량 요구/상황
  BooleanInput driveDemand       "주행 토크/전력 요구";
  BooleanInput regenDemand       "회생 가능(인버터로부터) 및 안전 조건 충족";
  BooleanInput needSCReplenish   "SC 보충 충전 필요(저전압/SoC_low 히스테리시스)";
    
     // 충전 관련
  BooleanInput dcChargeAllowed   "외부 DC충전 세션 허용(BMS 승인)";
  BooleanInput taperPhase        "배터리 CV 구간 진입 등 테이퍼 단계(정보용)"; 
  
    // ---- Outputs (현재 모드 표시용) ----
  BooleanOutput isIdle;
  BooleanOutput isPrecharge;
  BooleanOutput isDrive;
  BooleanOutput isRegen;
  BooleanOutput isDCCharge;
  BooleanOutput isSCReplenish;
  BooleanOutput isFault;  
    
    
    // ---- StateGraph nodes ----
    inner StateGraphRoot stateGraphRoot annotation(Placement(transformation(extent = {{41.91,309.9},{61.91,329.9}},rotation = 0.0,origin = {0.0,0.0})));
  // step
  ...
  Step Idle(start=start_in_idle)              "대기(정지/플러그 유무 무관)";
  StepWithSignal Drive         "주행(방전 중심)"                        annotation(Placement(transformation(extent = {{-30.59,307.69},{-10.59,327.69}},rotation = 0.0,origin = {0.0,0.0})));  
  StepWithSignal Regen         "회생(충전 중심)"                           annotation(Placement(transformation(extent = {{-30.59,307.69},{-10.59,327.69}},rotation = 0.0,origin = {0.0,0.0})));  
  StepWithSignal DCCharge      "외부 DC충전 세션"                           annotation(Placement(transformation(extent = {{-30.59,307.69},{-10.59,327.69}},rotation = 0.0,origin = {0.0,0.0})));  
  StepWithSignal SC_Replenish  "주행/충전 외 SC 보충 충전(배터리→SC)"       annotation(Placement(transformation(extent = {{-30.59,307.69},{-10.59,327.69}},rotation = 0.0,origin = {0.0,0.0})));  
  StepWithSignal Fault         "Fault 보존 상태(안전정지)"                 annotation(Placement(transformation(extent = {{-30.59,307.69},{-10.59,327.69}},rotation = 0.0,origin = {0.0,0.0})));
    
  // ---- Transitions with condition ports ----
  ...
  TransitionWithSignal t_Idle_to_Pre (use_conditionPort=true);
  TransitionWithSignal t_Pre_to_Idle (use_conditionPort=true);
  TransitionWithSignal t_Pre_to_Drive(use_conditionPort=true);
  TransitionWithSignal t_Pre_to_DC   (use_conditionPort=true);

  TransitionWithSignal t_Idle_to_Drive(use_conditionPort=true);
  TransitionWithSignal t_Idle_to_DC   (use_conditionPort=true);

  TransitionWithSignal t_Drive_to_Regen(use_conditionPort=true);
  TransitionWithSignal t_Regen_to_Drive(use_conditionPort=true);

  TransitionWithSignal t_Drive_to_SC  (use_conditionPort=true);
  TransitionWithSignal t_Regen_to_SC  (use_conditionPort=true);
  TransitionWithSignal t_SC_to_Drive  (use_conditionPort=true);

  TransitionWithSignal t_any_to_Fault(use_conditionPort=true);
  TransitionWithSignal t_Fault_to_Idle(use_conditionPort=true);
  ...
  .Electrification.Electrical.DCInit vInitA(init_steady=false, stateSelect=StateSelect.default,init_start = true,v_start = 200,common_mode = false) annotation (
    Placement(transformation(extent={{-152.44,-64.69},{-132.44,-44.69}},rotation = 0.0,origin = {0.0,0.0})));
    ...
    .Electrification.Converters.Control.Signals.i_b_ref i_b_ref(id = averageStepUpDownDCDC.id) annotation(Placement(transformation(extent = {{-149.65,24.56},{-141.65,32.56}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Electrification.Electrical.DCSensor dCSensor2 annotation(Placement(transformation(extent = {{-61.36,-61.35},{-41.36,-41.35}},origin = {0.0,0.0},rotation = 0.0)));
    .BatNSuperCap_legacy.AverageStepUpDownDCDC averageStepUpDownDCDC(enable_thermal_port = false,controller(external_mode = true,mode = .Electrification.Utilities.Types.ConverterMode.CurrentA,external_limits = true,listen = true,id_listen = SCap.id,typeListen = .Electrification.Utilities.Types.ControllerType.Battery,external_i_b = false,iMaxInB = 1000,external_i_a = false,pMaxInA = SCap.controller.controller1.pMaxIn * 100,pMaxInB = 1e16,external_pwr_b = true,external_pwr_a(fixed = false) = false,i_b_ref = 200 * (batty.i_nom_1C * 0.5),v_a_ref = 0,external_v_a = false,iMaxInA = SCap.controller.controller1.iMaxIn * 100,pwr_a_ref = 0,k_v = 10,aggregate_limits = false),common_mode = false,internal_ground = false,redeclare replaceable .Electrification.Converters.Electrical.Capacitor electrical_b,redeclare replaceable .Electrification.Converters.Electrical.Capacitor electrical_a(v_start = 0,v_start_fixed = false),redeclare replaceable .Electrification.Converters.Core.AverageStepUpDown core) annotation(Placement(transformation(extent = {{-109.24,-8.21},{-89.24,11.79}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Electrification.Electrical.DCInit vInitA3(init_steady = false,stateSelect = StateSelect.avoid,init_start = true,v_start = batty.OCV_start,common_mode = false) annotation(Placement(transformation(extent = {{-95.78,-147.7},{-75.78,-127.7}},rotation = 0.0,origin = {0.0,0.0})));
    .BatNSuperCap_legacy.BatteryChargerFixedLimits_CC_1 batteryChargerFixedLimits_CC_(display_name = true,redeclare replaceable .Electrification.Loads.Electrical.CapacitorAndDiode electrical,iMax = 300,enable_control_bus = false,vMax = 1000,pMax = 350000000) annotation(Placement(transformation(extent = {{6.04,-109.33},{26.04,-89.33}},origin = {0.0,0.0},rotation = 0.0)));
equation
    ...
    connect(close_contactors.systemBus,batty.controlBus) annotation(Line(points = {{-93.35,68.76},{-93.35,-86.21},{-124.13,-86.21}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(OnSC.y,i_b_ref.u_r) annotation(Line(points = {{-193.73,28.54},{-163.09,28.54},{-163.09,28.56},{-151.65,28.56}},color = {0,0,127}));
    ...
    connect(mode_ref.systemBus,averageStepUpDownDCDC.controlBus) annotation(Line(points = {{-120.63,78.82},{-107.24,78.82},{-107.24,11.79}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(averageStepUpDownDCDC.controlBus,SCap.controlBus) annotation(Line(points = {{-107.24,11.79},{-107.24,17.16},{-242.91,17.16},{-242.91,-33.12}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(i_b_ref.systemBus,averageStepUpDownDCDC.controlBus) annotation(Line(points = {{-141.65,28.56},{-107.24,28.56},{-107.24,11.79}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(averageStepUpDownDCDC.plug_a,dCSensor.plug_a) annotation(Line(points = {{-109.24,1.79},{-155.63,1.79},{-155.63,-42.88},{-199.99,-42.88}},color = {255,128,0}));
    connect(vInitA.plug_a,averageStepUpDownDCDC.plug_a) annotation(Line(points = {{-132.44,-54.69},{-122.61,-54.69},{-122.61,1.79},{-109.24,1.79}},color = {255,128,0}));
    ...
    connect(soC_cell.systemBus,batty.controlBus) annotation(Line(points = {{113.62,58.17},{113.62,-13.94},{-124.13,-13.94},{-124.13,-86.21}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(absI.u,dCSensor2.y[2]) annotation(Line(points = {{-49.84,140.63},{-49.84,-34.68},{-35.36,-34.68},{-35.36,-66.68},{-51.36,-66.68},{-51.36,-60.35}},color = {0,0,127}));
    ...
    connect(dCSensor2.plug_a,batty.plug_a) annotation(Line(points = {{-61.36,-51.35},{-87.78,-51.35},{-87.78,-102.21},{-120.93,-102.21}},color = {255,128,0}));
    connect(batteryChargerFixedLimits_CC_.plug_a,dCSensor2.plug_b) annotation(Line(points = {{6.04,-99.33},{-17.66,-99.33},{-17.66,-51.35},{-41.36,-51.35}},color = {255,128,0}));
    connect(averageStepUpDownDCDC.plug_b,batteryChargerFixedLimits_CC_.plug_a) annotation(Line(points = {{-89.24,1.79},{-10.95,1.79},{-10.95,-99.33},{6.04,-99.33}},color = {255,128,0}));
    connect(vInitA3.plug_a,batty.plug_a) annotation(Line(points = {{-75.78,-137.7},{-69.78,-137.7},{-69.78,-102.21},{-120.93,-102.21}},color = {255,128,0}));
...
end Cycle_HESS_EMS_charge_wCommu;
