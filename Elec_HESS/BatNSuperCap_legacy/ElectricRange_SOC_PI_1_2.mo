...
model ElectricRange_SOC_PI_1_2 "Range of SCap electric vehicle for a specific drive cycle"
  ...
  .Electrification.Electrical.DCInit vInitA(init_steady=false, stateSelect=StateSelect.default,init_start = true,v_start = 200,common_mode = false) annotation (
    ...
    .Modelica.StateGraph.TransitionWithSignal HighCurrent(waitTime = 0.3,enableTimer = true) annotation(Placement(transformation(extent = {{-59.95,311.11},{-39.95,331.11}},rotation = 0.0,origin = {0.0,0.0})));
    ...
    .Modelica.StateGraph.TransitionWithSignal lowCurrentSCap(enableTimer = false,waitTime = 2) annotation(Placement(transformation(extent = {{-64.79,354.08},{-84.79,374.08}},rotation = 0.0,origin = {0.0,0.0})));
    ...
    .Electrification.Electrical.DCSensor dCSensor2 annotation(Placement(transformation(extent = {{-30.86,-109.37},{-50.86,-89.37}},origin = {0.0,0.0},rotation = 0.0)));
    .BatNSuperCap_legacy.AverageStepUpDownDCDC CapDCDC(enable_thermal_port = false,redeclare replaceable .Electrification.Converters.Core.Isolated core,display_name = true,redeclare replaceable .Electrification.Converters.Control.Voltage controller(external_v_b = false,mode = Electrification.Utilities.Types.ConverterModeVoltage.VoltageB,v_b_ref = 200)) annotation(Placement(transformation(extent = {{-128.23,-19.81},{-148.23,0.19}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Modelica.Blocks.Logical.Hysteresis hysteresis2(uLow = 0.45,uHigh = 0.5) annotation(Placement(transformation(extent = {{-62.02,237.58},{-42.02,257.58}},origin = {0.0,0.0},rotation = 0.0)));
equation
    ...
    connect(motor.plug_a,dCSensor2.plug_a) annotation(Line(points = {{43.18,-29.15},{6.31,-29.15},{6.31,-99.37},{-30.86,-99.37}},color = {255,128,0}));
    connect(dCSensor2.plug_b,batty.plug_a) annotation(Line(points = {{-50.86,-99.37},{-67.16,-99.37},{-67.16,-100.27},{-83.75,-100.27}},color = {255,128,0}));
    ...
    connect(motor.controlBus,pwr_sns.systemBus) annotation(Line(points = {{45.73,-19.15},{45.73,16.89},{-25.48,16.89},{-25.48,13.33}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(pwr_sns.y_r,pwr_b_ref.u_r) annotation(Line(points = {{-34.48,13.33},{-73.06,13.33},{-73.06,21.03},{-111.91,21.03}},color = {0,0,127}));
    connect(dCSensor2.y[2],absI.u) annotation(Line(points = {{-40.86,-108.37},{-40.86,-114.66},{-56.57,-114.66},{-56.57,-82.66},{-49.84,-82.66},{-49.84,140.99}},color = {0,0,127}));
    connect(dCSensor.y[2],absI2.u) annotation(Line(points = {{-210.56,-49.59},{-210.56,-55.59},{-277.45,-55.59},{-277.45,176.44}},color = {0,0,127}));
    connect(convCrate.y,hysteresis2.u) annotation(Line(points = {{-48.76,204.02},{-48.76,210.02},{-70.02,210.02},{-70.02,247.58},{-64.02,247.58}},color = {0,0,127}));
    connect(hysteresis2.y,HighCurrent.condition) annotation(Line(points = {{-41.02,247.58},{-35.02,247.58},{-35.02,303.11},{-49.95,303.11},{-49.95,309.11}},color = {255,0,255}));
