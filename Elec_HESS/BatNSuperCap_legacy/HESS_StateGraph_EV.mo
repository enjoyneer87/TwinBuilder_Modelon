...
model HESS_StateGraph_EV "Range of SCap electric vehicle for a specific drive cycle"
  ...
  .Electrification.Electrical.DCInit vInitA(init_steady=true, stateSelect=StateSelect.default,init_start = false,v_start = SCap.summary.ocv,common_mode = false) annotation (
    ...
    .BatNSuperCap_legacy.AverageStepUpDownDCDC averageStepUpDownDCDC(enable_thermal_port = false,controller(external_mode = false,mode = Electrification.Utilities.Types.ConverterMode.PowerA,i_a_ref = -SCap.i_nom_1C,external_limits = true,listen = true,id_listen = SCap.id,typeListen = .Electrification.Utilities.Types.ControllerType.Battery,external_i_b = false,iMaxInB = 500,external_i_a = false,i_b_ref = -SCap.i_nom_1C,iMaxInA = 200,pMaxInA = 1e16,pMaxInB = 1e16,external_pwr_b = false,external_pwr_a = true,pwr_a_ref = torqueArbitration.number_of_motors_rear),common_mode = false,internal_ground = false) annotation(Placement(transformation(extent = {{-109.11,-14.34},{-89.11,5.66}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Modelica.Blocks.Math.BooleanToInteger booleanToInteger(integerFalse = Integer(.Electrification.Utilities.Types.ConverterMode.Disabled),integerTrue = Integer(.Electrification.Utilities.Types.ConverterMode.PowerA)) annotation(Placement(transformation(extent = {{-180.83,54.36},{-160.83,74.36}},origin = {0.0,0.0},rotation = 0.0)));
...
equation
    ...
    connect(booleanToInteger.y,mode_ref.u_i) annotation(Line(points = {{-159.83,64.36},{-133.95,64.36},{-133.95,63.06}},color = {255,127,0}));
    connect(mode_ref.systemBus,averageStepUpDownDCDC.controlBus) annotation(Line(points = {{-123.95,63.06},{-107.11,63.06},{-107.11,5.66}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(close_contactors.systemBus,batty.controlBus) annotation(Line(points = {{-92.97,161.41},{-76.81,161.41},{-76.81,-96.35},{-43.41,-96.35}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(hysteresis2.y,OnSC.u) annotation(Line(points = {{-49.64,264.75},{-49.64,273.44},{-120.24,273.44},{-120.24,117.44}},color = {255,0,255}));
    ...
    connect(averageStepUpDownDCDC.controlBus,SCap.controlBus) annotation(Line(points = {{-107.11,5.66},{-107.11,17.02},{-148.39,17.02},{-148.39,10.39}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(SCon.active,booleanToInteger.u) annotation(Line(points = {{-9.13,310.97},{-9.13,303.61},{-188,303.61},{-188,64.36},{-182.83,64.36}},color = {255,0,255}));
    ...
    connect(tau_ref.systemBus,machine.controlBus) annotation(Line(points = {{67.63,62.7},{50.76,62.7},{50.76,11.66}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(machine.plug_a,dCSensor.plug_b) annotation(Line(points = {{47.56,-4.34},{32.43,-4.34},{32.43,-4.02},{14.29,-4.02}},color = {255,128,0}));
    connect(machine.plug_a,batty.plug_a) annotation(Line(points = {{47.56,-4.34},{47.56,-112.35},{-40.21,-112.35}},color = {255,128,0}));
    ...
    connect(SCap.controlBus,soc_SC.systemBus) annotation(Line(points = {{-148.39,10.39},{-148.39,21.84},{101.53,21.84},{101.53,172.27},{97.13,172.27}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(machine.flange,brakes.flange_a) annotation(Line(points = {{79.56,-4.34},{121.16,-4.34},{121.16,1.18}},color = {0,0,0}));
    connect(averageStepUpDownDCDC.plug_a,SCap.plug_a) annotation(Line(points = {{-109.11,-4.34},{-127.38,-4.34},{-127.38,0.39},{-146.39,0.39}},color = {255,128,0}));
    connect(dCSensor.plug_a,averageStepUpDownDCDC.plug_b) annotation(Line(points = {{-5.71,-4.02},{-46.87,-4.02},{-46.87,-4.34},{-89.11,-4.34}},color = {255,128,0}));
    connect(machine.controlBus,batty.controlBus) annotation(Line(points = {{50.76,11.66},{50.76,17.66},{-43.41,17.66},{-43.41,-96.35}},color = {240,170,40},pattern = LinePattern.Dot));
