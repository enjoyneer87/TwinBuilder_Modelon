...
model HESS_CrateHysteresis_StateGraph_Fix_Charger_2
  ...
  .Electrification.Electrical.DCSensor dCSensor
    ...
    .Modelica.Blocks.Logical.Less less2 annotation(Placement(transformation(extent = {{-19.06,-152.03},{0.94,-132.03}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Electrification.Electrical.DCInit vInitB2(init_start = true,v_start = SuperCap.OCV_start) annotation(Placement(transformation(extent = {{339.02,-21.07},{351.02,-9.07}},rotation = 0.0,origin = {0.0,0.0})));
    .Modelica.Blocks.Discrete.ZeroOrderHold zeroOrderHold(samplePeriod = 0.01,startTime = 500) annotation(Placement(transformation(extent = {{1.91,-201.59},{21.91,-181.59}},origin = {0,0},rotation = 0)));
    .Modelica.Blocks.Math.IntegerToReal integerToReal annotation(Placement(transformation(extent = {{-35.71,-201.59},{-15.71,-181.59}},origin = {0,0},rotation = 0)));
    .Modelica.Blocks.Math.RealToInteger realToInteger annotation(Placement(transformation(extent = {{32.37,-202.48},{52.37,-182.48}},origin = {0,0},rotation = 0)));
...
equation
  connect(dCSensor.plug_b,battery.plug_a)
    ...
    connect(getComponentLimits.i_max_in,less.u2) annotation(Line(points = {{157,128},{-44.71,128},{-44.71,134.19},{-38,134.19}},color = {0,0,127}));
    ...
    connect(getComponentLimits4Battery.i_max_in,less2.u2) annotation(Line(points = {{161.2,-233.11},{130.65,-233.11},{130.65,-164.1},{-38.53,-164.1},{-38.53,-150.03},{-21.06,-150.03}},color = {0,0,127}));
    connect(gain.y,less2.u1) annotation(Line(points = {{-104.29,-92.76},{-63.79,-92.76},{-63.79,-142.03},{-21.06,-142.03}},color = {0,0,127}));
    connect(less2.y,and3.u2) annotation(Line(points = {{1.94,-142.03},{11.94,-142.03},{11.94,-146.97}},color = {255,0,255}));
    ...
    connect(transitionWithSignal2.outPort,stepCap.inPort[2]) annotation(Line(points = {{2.67,92},{-41,92},{-41,60},{-34.78,60}},color = {0,0,0}));
    ...
    connect(dCSensor2.plug_a,CapDCDC.plug_b) annotation(Line(points = {{347.46,-53.4},{347.46,-54},{322.26,-54}},color = {255,128,0}));
    connect(CapDCDC.plug_a,batteryChargerFixedLimits_CC_.plug_a) annotation(Line(points = {{302.26,-54},{-69.55,-54},{-69.55,-76.41},{-240.93,-76.41}},color = {255,128,0}));
    connect(SuperCap.controlBus,CapDCDC.controlBus) annotation(Line(points = {{379.46,-43.4},{379.46,-37.81},{304.26,-37.81},{304.26,-44}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(vInitB2.plug_a,SuperCap.plug_a) annotation(Line(points = {{351.02,-15.07},{372.52,-15.07},{372.52,-53.4},{377.46,-53.4}},color = {255,128,0}));
    connect(integerToReal.y,zeroOrderHold.u) annotation(Line(points = {{-14.71,-191.59},{-0.09,-191.59}},color = {0,0,127}));
    connect(booleanToInteger.y,integerToReal.u) annotation(Line(points = {{12.49,-170.78},{18.49,-170.78},{18.49,-175.57},{-41.71,-175.57},{-41.71,-191.59},{-37.71,-191.59}},color = {255,127,0}));
    connect(zeroOrderHold.y,realToInteger.u) annotation(Line(points = {{22.91,-191.59},{26.64,-191.59},{26.64,-192.48},{30.37,-192.48}},color = {0,0,127}));
    connect(realToInteger.y,mode_ref.u_i) annotation(Line(points = {{53.37,-192.48},{59.37,-192.48},{59.37,-208.48},{19.559999999999995,-208.48},{19.559999999999995,-170.78},{23.56,-170.78}},color = {255,127,0}));
...
end HESS_CrateHysteresis_StateGraph_Fix_Charger_2;
