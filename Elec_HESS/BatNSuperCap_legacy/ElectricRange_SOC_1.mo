...
model ElectricRange_SOC_1 "Range of SCap electric vehicle for a specific drive cycle"
  ...
  .Electrification.Electrical.DCInit vInitA(init_steady=false, stateSelect=StateSelect.default,init_start = true,v_start = 200,common_mode = false) annotation (
    ...
    .Modelica.StateGraph.TransitionWithSignal lowCurrentSCap(enableTimer = false) annotation(Placement(transformation(extent = {{-68.2,310.03},{-48.2,330.03}},rotation = 0.0,origin = {0.0,0.0})));
    ...
    .Modelica.Blocks.Logical.GreaterThreshold greaterThreshold2(threshold = 0.9) annotation(Placement(transformation(extent = {{-10.0,-10.0},{10.0,10.0}},origin = {114.66,102.5},rotation = 90.0)));
    ...
    .BatNSuperCap_legacy.AverageStepUpDownDCDC DCDC_Bat(redeclare replaceable .Electrification.Converters.Core.AverageStepUpDown core,redeclare replaceable .Electrification.Converters.Electrical.Capacitor electrical_a(v_start_fixed = false,v_start = 0),redeclare replaceable .Electrification.Converters.Electrical.Capacitor electrical_b,internal_ground = false,common_mode = false,controller(external_mode = false,mode = Electrification.Utilities.Types.ConverterMode.VoltageA,external_limits = false,listen = true,id_listen = SCap.id,typeListen = .Electrification.Utilities.Types.ControllerType.Battery,external_i_b = false,iMaxInB = 1000,external_i_a = false,pMaxInA = SCap.controller.controller1.pMaxIn * 100,pMaxInB = 1e16,external_pwr_b = false,external_pwr_a(fixed = false) = false,i_b_ref = -SCap.i_nom_1C,v_a_ref = 400,external_v_a = false,iMaxInA = batty.i_nom_1C * 0.5,i_a_ref = SCap.controller.controller1.iMaxIn,pwr_a_ref = 0,k_v = 10,aggregate_limits = false),enable_thermal_port = false) annotation(Placement(transformation(extent = {{-93.47,-111.26},{-73.47,-91.26}},origin = {0.0,0.0},rotation = 0.0)));
...
equation
    ...
    connect(Bat.active,close_contactors.u_b) annotation(Line(points = {{-19.85,308.92},{-19.85,103.95},{-93.35,103.95},{-93.35,89.38}},color = {255,0,255}));
    ...
    connect(close_contactors.systemBus,batty.controlBus) annotation(Line(points = {{-93.35,68.76},{-93.35,-85.18},{-117.41,-85.18}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(not1.y,lowCurrentSCap.condition) annotation(Line(points = {{-181.24,273.02},{-58.2,273.02},{-58.2,308.03}},color = {255,0,255}));
    ...
    connect(mode_ref.systemBus,averageStepUpDownDCDC.controlBus) annotation(Line(points = {{-120.63,78.82},{-107.5,78.82},{-107.5,11.16}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(averageStepUpDownDCDC.controlBus,SCap.controlBus) annotation(Line(points = {{-107.5,11.16},{-107.5,17.16},{-242.91,17.16},{-242.91,-33.12}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(i_b_ref.systemBus,averageStepUpDownDCDC.controlBus) annotation(Line(points = {{-141.41,28.56},{-107.5,28.56},{-107.5,11.16}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(averageStepUpDownDCDC.plug_a,dCSensor.plug_a) annotation(Line(points = {{-109.5,1.16},{-155.63,1.16},{-155.63,-42.88},{-199.99,-42.88}},color = {255,128,0}));
    connect(vInitA.plug_a,averageStepUpDownDCDC.plug_a) annotation(Line(points = {{-132.44,-55.02},{-122.61,-55.02},{-122.61,1.16},{-109.5,1.16}},color = {255,128,0}));
    connect(pwr_a_ref.systemBus,averageStepUpDownDCDC.controlBus) annotation(Line(points = {{-200.9,6.96},{-129.06,6.96},{-129.06,17.45},{-107.5,17.45},{-107.5,11.16}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(averageStepUpDownDCDC.plug_b,machine.plug_a) annotation(Line(points = {{-89.5,1.16},{31.14,1.16},{31.14,-117.78},{54.6,-117.78}},color = {255,128,0}));
    ...
    connect(pwr_b_ref.systemBus,averageStepUpDownDCDC.controlBus) annotation(Line(points = {{-201.75,-3.89},{-195.75,-3.89},{-195.75,17.45},{-107.5,17.45},{-107.5,11.16}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(batty.controlBus,soc.systemBus) annotation(Line(points = {{-117.41,-85.18},{-117.41,60.85},{-321.57,60.85}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(soC_cell.y_r,greaterThreshold2.u) annotation(Line(points = {{113.62,67.17},{113.62,78.91},{114.66,78.91},{114.66,90.5}},color = {0,0,127}));
    ...
    connect(greaterThreshold2.y,or1.u2) annotation(Line(points = {{114.66,113.5},{114.66,118.97},{133.02,118.97},{133.02,194.29},{127.02,194.29}},color = {255,0,255}));
    ...
    connect(soC_cell.systemBus,batty.controlBus) annotation(Line(points = {{113.62,58.17},{113.62,-13.94},{-117.41,-13.94},{-117.41,-85.18}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(lowCurrentSCap.outPort,Bat.inPort[1]) annotation(Line(points = {{-56.7,320.03},{-43.775000000000006,320.03},{-43.775000000000006,319.92},{-30.85,319.92}},color = {0,0,0}));
    ...
    connect(vInitA2.plug_a,DCDC_Bat.plug_a) annotation(Line(points = {{-88.86,-149.21},{-82.86,-149.21},{-82.86,-138.97},{-93.47,-138.97},{-93.47,-101.26}},color = {255,128,0}));
    connect(batty.plug_a,DCDC_Bat.plug_a) annotation(Line(points = {{-114.21,-101.18},{-106.21,-101.18},{-106.21,-101.26},{-93.47,-101.26}},color = {255,128,0}));
    connect(dCSensor2.plug_a,DCDC_Bat.plug_b) annotation(Line(points = {{-61.36,-51.68},{-69.79,-51.68},{-69.79,-101.26},{-73.47,-101.26}},color = {255,128,0}));
    ...
    connect(SCon.outPort[1],lowCurrentSCap.inPort) annotation(Line(points = {{-150.81,317.26},{-106.505,317.26},{-106.505,320.03},{-62.2,320.03}},color = {0,0,0}));
