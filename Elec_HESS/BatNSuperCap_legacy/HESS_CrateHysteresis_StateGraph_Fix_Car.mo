...
model HESS_CrateHysteresis_StateGraph_Fix_Car
  ...
  .Electrification.Electrical.DCSensor dCSensor
    ...
    .Electrification.Control.Limits.ListenToLimits listenToLimits(typeListen = Electrification.Utilities.Types.ControllerType.Battery,id_listen = battery.id,vMaxSignal = true,vMinSignal = true,iMaxOutSignal = true,iMaxInSignal = true,pMaxOutSignal = true,pMaxInSignal = true) annotation(Placement(transformation(extent = {{-208.0,-192.0},{-188.0,-172.0}},origin = {0.0,0.0},rotation = 0.0)));
    .Electrification.Control.Interfaces.SystemBus controlBus annotation(Placement(transformation(extent = {{-246.27,-164.0},{-206.27,-124.0}},origin = {0.0,0.0},rotation = 0.0)));
...
equation
  connect(dCSensor.plug_b,battery.plug_a)
    ...
    connect(batteryDCDC.plug_a,dCSensor.plug_a) annotation(Line(points = {{322,-98},{347.48,-98},{347.48,-98.35}},color = {255,128,0}));
    ...
    connect(listenToLimits.signalsBus,controlBus) annotation(Line(points = {{-198,-192},{-198,-198},{-226.27,-198},{-226.27,-144}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(machine.controlBus,controlBus) annotation(Line(points = {{-285.04,-62},{-285.04,-56},{-226.27,-56},{-226.27,-144}},color = {240,170,40},pattern = LinePattern.Dot));
