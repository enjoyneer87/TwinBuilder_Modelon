...
model CrateHysteresis_StateGraph_W_NonInit
  ...
  .BatNSuperCap_legacy.BatFromCar_ContactNLimit battery(
    ...
    annotation(Placement(transformation(extent = {{172.92,-19.92},{192.92,0.08}},rotation = 0.0,origin = {0.0,0.0})));
  ...
  .Electrification.Electrical.DCSensor dCSensor
    ...
    .Modelica.Blocks.Logical.LessThreshold lessThreshold(threshold = 0.5 - 0.01) annotation(Placement(transformation(extent = {{-36.0,-16.0},{-16.0,4.0}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Modelica.StateGraph.StepWithSignal stepBattery(nIn = 1,nOut = 1) annotation(Placement(transformation(extent = {{24.46,50.51},{44.46,70.51}},origin = {0.0,0.0},rotation = 0.0)));
    .Modelica.StateGraph.TransitionWithSignal transitionWithSignal(enableTimer = true,waitTime = 0.2) annotation(Placement(transformation(extent = {{-10.0,50.0},{10.0,70.0}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Modelica.StateGraph.TransitionWithSignal CVOn(waitTime = 0,enableTimer = true) annotation(Placement(transformation(extent = {{51.15,57.15},{68.85,74.85}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Electrification.Electrical.DCInit vInit(init_start = true,v_start = battery2.OCV_start) annotation(Placement(transformation(extent = {{186.89,6.89},{206.89,26.89}},origin = {0.0,0.0},rotation = 0.0)));
    .Electrification.Electrical.DCInit vInit2(v_start = battery.OCV_start,init_start = true) annotation(Placement(transformation(extent = {{161.26,-41.79},{181.26,-21.79}},origin = {0.0,0.0},rotation = 0.0)));

equation
  connect(dCSensor.plug_b,battery.plug_a)
    annotation(Line(points = {{114,-12},{172.92,-12},{172.92,-9.92}},color = {255,128,0}));
    ...
    connect(battery.controlBus,getComponentLimits2.componentBus) annotation(Line(points = {{174.92,0.08},{174.92,4},{136,4},{136,-62},{148,-62}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(stepBattery.active,and3.u1) annotation(Line(points = {{34.46,49.51},{34.46,26},{40.34,26}},color = {255,0,255}));
    ...
    connect(transitionWithSignal.outPort,stepBattery.inPort[1]) annotation(Line(points = {{1.5,60},{23.46,60},{23.46,60.51}},color = {0,0,0}));
    ...
    connect(close_contactors.systemBus,battery.controlBus) annotation(Line(points = {{56,4},{174.92,4},{174.92,0.08}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(batteryDCDC.plug_a,batteryChargerFixedLimits_CC_.plug_a) annotation(Line(points = {{44,-38},{-50,-38},{-50,-62},{-44,-62}},color = {255,128,0}));
    connect(batteryDCDC.controlBus,battery.controlBus) annotation(Line(points = {{46,-28},{46,4},{174.92,4},{174.92,0.08}},color = {240,170,40},pattern = LinePattern.Dot));
    ...
    connect(CapDCDC.plug_a,batteryChargerFixedLimits_CC_.plug_a) annotation(Line(points = {{100,34},{-50,34},{-50,-62},{-44,-62}},color = {255,128,0}));
    connect(battery2.controlBus,CapDCDC.controlBus) annotation(Line(points = {{160,44},{160,50},{102,50},{102,44}},color = {240,170,40},pattern = LinePattern.Dot));
    connect(dCSensor.y[1],less3.u1) annotation(Line(points = {{104,-21},{104,-114},{-32,-114},{-32,-80},{-20,-80}},color = {0,0,127}));
    connect(getComponentLimits2.v_max,less3.u2) annotation(Line(points = {{127,-46},{121,-46},{121,-100},{-28,-100},{-28,-88},{-20,-88}},color = {0,0,127}));
    connect(transitionWithSignal2.inPort,CV.outPort[1]) annotation(Line(points = {{7.54,92},{98.5,92},{98.5,66},{92.5,66}},color = {0,0,0}));
    connect(stepBattery.outPort[1],CVOn.inPort) annotation(Line(points = {{44.96,60.51},{44.96,66},{56.46,66}},color = {0,0,0}));
    ...
    connect(vInit.plug_a,battery2.plug_a) annotation(Line(points = {{206.89,16.89},{168.77,16.89},{168.77,10.33},{151.86,10.33},{151.86,34},{158,34}},color = {255,128,0}));
    connect(vInit2.plug_a,battery.plug_a) annotation(Line(points = {{181.26,-31.79},{187.38,-31.79},{187.38,-47.79},{155.38,-47.79},{155.38,-9.92},{172.92,-9.92}},color = {255,128,0}));
...
end CrateHysteresis_StateGraph_W_NonInit;
